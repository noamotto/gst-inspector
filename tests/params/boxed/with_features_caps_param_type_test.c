...
#define FEATURES ("test:TestSomething")
...
static void check_parse(const GValue *value)
{
    ...
    g_assert_cmpuint(gst_value_array_get_size(value), ==, 1);

    caps_value = gst_value_array_get_value(value, 0);
    ...
    g_assert_true(gst_structure_has_field_typed(caps_struct, KEY_FEATURES, G_TYPE_STRING));
    g_assert_cmpstr(gst_dictionary_get_string(caps_struct, KEY_FEATURES), ==, FEATURES);
    
    g_assert_true(gst_structure_has_field_typed(caps_struct, "Caps", GST_TYPE_ARRAY));
